...
import { SpriteAttributePane } from "./SpriteAttributePane.jsx"; //height and stuff
...
export default function SpritePane(){
    const { pyatchVM, targetIds, editingTargetId } = useContext(patchContext);
    if (!pyatchVM) {
        return null;
    }
        ...
        <Grid>
            ...
            <Grid container direction="row" sx={{ alignItems: 'center', justifyItems: 'center' }}>
                {editingTarget && <Grid item xs={12}>
                    <SpriteName target={editingTarget} />
                </Grid>}
            </Grid>
