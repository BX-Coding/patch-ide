...
class PrimProxy {
    ...
    static SOUND_EFFECT_LIST = ["pitch", "pan"];

    static MOTION_ROTATION_STYLES = ["all-around", "left-right", "none"];

    static LOOKS_EFFECT_LIST = ["color", "fisheye", "whirl", "pixelate", "mosaic", "brightness", "ghost"];

    static SENSING_CURRENT_INCREMENT_OPTIONS = ["year", "month", "date", "dayofweek", "hour", "minute", "second"];

    static CONTROL_STOP_OPTIONS = ["all", "other", "this"];

    static PEN_EFFECT_LIST = ["color", "saturation", "brightness", "transparency"];

    static patchApi: Dictionary<any> = {
        move: {
        ...
        setRotationStyle: {
            ...
            parameters: ["style"],
            exampleParameters: { style: PrimProxy.MOTION_ROTATION_STYLES },
        },
        changeX: {
        ...
        changeGraphicEffectBy: {
            ...
            exampleParameters: { effect: PrimProxy.LOOKS_EFFECT_LIST, change: PrimProxy.DEFAULT_CHANGE },
        },
        setGraphicEffectTo: {
            opcode: "looks_seteffectto",
            parameters: ["effect", "value"],
            exampleParameters: { effect: PrimProxy.LOOKS_EFFECT_LIST, value: PrimProxy.DEFAULT_EFFECT },
        },
        clearGraphicEffects: {
        ...
        setSoundEffectTo: {
            ...
            parameters: ["effect", "value"],
            exampleParameters: { effect: PrimProxy.SOUND_EFFECT_LIST, value: PrimProxy.DEFAULT_EFFECT },
        },
        changeSoundEffectBy: {
            opcode: "sound_changeeffectby",
            parameters: ["effect", "change"],
            exampleParameters: { effect: PrimProxy.SOUND_EFFECT_LIST, change: PrimProxy.DEFAULT_CHANGE },
        },
        clearSoundEffects: {
        ...
        current: {
            ...
            exampleParameters: { timeIncrement: PrimProxy.SENSING_CURRENT_INCREMENT_OPTIONS },
        },
        daysSince2000: {
        ...
        stop: {
            opcode: "control_stop",
            parameters: ["option"],
            exampleParameters: { option: PrimProxy.CONTROL_STOP_OPTIONS },
        },
        createClone: {
        ...
        changePenEffect: {
            ...
            parameters: ["effect", "change"],
            exampleParameters: { effect: PrimProxy.PEN_EFFECT_LIST, change: PrimProxy.DEFAULT_CHANGE },
        },
        setPenEffect: {
            opcode: "pen_setpencolorparamto",
            parameters: ["effect", "value"],
            exampleParameters: { effect: PrimProxy.PEN_EFFECT_LIST, value: PrimProxy.DEFAULT_EFFECT },
        },
        changePenSize: {
    ...
    static interruptMap = {};
    threadId: string;
    post: (opCode: any, args: any) => Promise<any>;

    constructor(threadId: string, interruptFunction: any, postFunction: any) {
    ...
    static fillDynamicOptions(parameterOptions: any[], vm: { getAllRenderedTargets: () => any[]; runtime: { getTargetForStage: () => { (): any; new(): any; sprite: { (): any; new(): any; costumes: any[]; }; }; }; editingTarget: { sprite: { costumes: any[]; }; getSounds: () => any[]; }; getAllBroadcastMessages: () => any; }) {
        const dynamicOptions: any[] = [];
        parameterOptions.forEach((option: string) => {
            if (option === PrimProxy.TARGET_NAMES) {
                const targets = vm.getAllRenderedTargets().filter((target: { isStage: any; }) => !target.isStage);
                const targetNames = targets.map((target: { getName: () => any; }) => target.getName());
                dynamicOptions.push(...targetNames);
            } else if (option === PrimProxy.BACKDROP_NAMES) {
                const backdropNames = vm.runtime.getTargetForStage().sprite.costumes.map((costume: { name: any; }) => costume.name);
                dynamicOptions.push(...backdropNames);
            } else if (option === PrimProxy.COSTUME_NAMES) {
                const costumeNames = vm.editingTarget.sprite.costumes.map((costume: { name: any; }) => costume.name);
                dynamicOptions.push(...costumeNames);
            } else if (option === PrimProxy.SOUND_NAMES) {
                const soundNames = vm.editingTarget.getSounds().map((sound: { name: any; }) => sound.name);
    ...
    static getDynamicFunctionInfo(functionName: string | number, vm: any) {
        const functionInfo = _.cloneDeep(PrimProxy.patchApi[functionName]);
        if (!functionInfo) {
            console.warn(`Function ${functionName} not found`);
        }
        functionInfo.parameters.forEach((param: string | number) => {
    ...
    async changeGraphicEffectBy(effect: any, change: any) {
    ...
    async setGraphicEffectTo(effect: any, value: any) {
    ...
    async getBackdrop() {
        const backdrop = await this.post(PrimProxy.patchApi.getBackdrop.opcode, {
            NUMBER_NAME: "name",
        });
        return backdrop;
    }

    async playSound(soundMenu: any) {
        await this.post(PrimProxy.patchApi.playSound.opcode, { SOUND_MENU: soundMenu });
    }

    async playSoundUntilDone(soundMenu: any) {
    ...
    async setSoundEffectTo(effect: any, value: any) {
        await this.post(PrimProxy.patchApi.setSoundEffectTo.opcode, { EFFECT: effect, VALUE: value });
    }

    async changeSoundEffectBy(effect: any, value: any) {
    ...
    async getAttributeOf(object: any, property: any) {
