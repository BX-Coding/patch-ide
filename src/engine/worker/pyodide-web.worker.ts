...
import { loadPyodide, version as npmVersion, PyodideInterface } from "pyodide";
 ...
 * @private
 */
const _pendingTokens: Dictionary<(value: any) => void> = {};

/**
 * Final token of the last run.
 * @type {string}
 */
const _lastTokens: Dictionary<string> = {};
 ...
 * @type {Object}
 */
//let _initPyodideState: any = null;

/**
 * Dict of threadId and threadFunc
 * @type {Object}
 */
// const _threads: object = {};
...
function _postMessage(id: string, threadId: string | null, opCode: string | null, args: any, token: string | null) {
    _postWorkerMessage({ id, threadId, opCode, args, token });
}

function _postStatusMessage(id: string) {
    _postMessage(id, null, null, null, null);
}

function _postLoadStatusMessage(id: string, loadPromiseId: any) {
    _postWorkerMessage({ id, loadPromiseId });
}

function _postThreadStatusMessage(id: any, threadId: any) {
    _postWorkerMessage({ id, threadId });
}

function _postRuntimeError(threadId: any, lineNumber: any, message: any) {
    _postWorkerMessage({ id: WorkerMessages.ToVM.PythonError, threadId, lineNumber, message, type: "RuntimeError" });
}

function _postCompileTimeError(threadId: any, lineNumber: any, message: any, loadPromiseId: any) {
    _postWorkerMessage({ id: WorkerMessages.ToVM.PythonError, threadId, lineNumber, message, type: "CompileTimeError", loadPromiseId });
}

async function _initPyodide(interruptBuffer: any) {
    ...
    // TODO: is this needed? it doesn't work.
...
function _resolvePendingToken(token: string | number, value: any) {
    if (_pendingTokens) {
...
function _postBlockOpMessage(threadId: any, opCode: any, args: {}) {
...
function parseRuntimePythonError(error: { message: any; }) {
...
function parseCompileTimePythonError(error: Error) {
    const errorMessage = error.message;
    // Extract line number using regex
...
function _loadGlobal(script: any, loadPromiseId: any, threadId: any) {
    try {
        self.pyodide.globals.set(`thread_${threadId}`, null);
        self.pyodide.runPython(script);
    } catch (error) {
        const { lineNumber, errorLineMessage } = parseCompileTimePythonError(error as Error);
...
function _getThreadFunction(threadId: any) {
...
function _startThread(threadId: string) {
    const endThreadPost = (_threadId: any) => {
        _postBlockOpMessage(_threadId, PrimProxy.patchApi.endThread.opcode, {});
    };
    const handleRuntimeError = (_threadId: any) => (error: { message: string | string[]; }) => {
...
function onVMMessage(event: { data: { id?: any; token?: any; value?: any; threadId?: any; interruptBuffer?: any; script?: any; loadPromiseId?: any; }; }) {
